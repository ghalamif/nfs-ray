...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, label  # Return the transformed image and label
...
def train_func():
    ...
    model = models.resnet18(num_classes=2)  # Load a ResNet-18 model
    ...
    dataset = CustomImageDataset(data_dir, labels_path, transform=transform)
    ...
    # Training loop
    scaler = torch.amp.GradScaler(device='cuda')  # Mixed precision scaler
...
def train_loop_per_worker(config):
    ...
    # Set up the model, loss function, and optimizer
    model = models.resnet18(weights="IMAGENET1K_V1")  # Load a pre-trained ResNet-18 model
    num_ftrs = model.fc.in_features  # Get the number of input features for the fully connected layer
    model.fc = nn.Linear(num_ftrs, 2)  # Replace the fully connected layer to match the number of classes (OK, NOK)
