...
# Define the root directory and the directory to keep
root_directory = "/srv/nfs/kube-ray/storage"
keep_directory = os.path.join(root_directory, "checkpoints")
...
for item in os.listdir(root_directory):
    ...
    if os.path.isfile(item_path) or os.path.islink(item_path):
        ...
        shutil.rmtree(item_path)  # Remove the entire directory

print("Clean up completed, only 'checkpoints' folder is retained.")
...
train_config = {
    "lr": tune.loguniform(1e-5, 1e-1),  # Learning rate range for tuning
    "epochs": 12,                       # Fixed number of epochs
    "batch_size_per_worker": 16,        # Batch size for each worker
}
...
# [6] Using Tuner for hyperparameter tuning
# Use Tuner to manage hyperparameter tuning and interaction with TorchTrainer

tuner = Tuner(
    trainer,
    param_space={
            "train_loop_config": {
                "lr": tune.loguniform(1e-5, 1e-1), #tune.grid_search([0.001, 0.01, 0.05, 0.1]),
                "batch_size_per_worker": total_batch_size // num_workers,
                "epochs": 10,
            }
        },  
